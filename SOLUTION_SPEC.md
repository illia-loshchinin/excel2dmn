...
| *M+2* | **Type** — `typeRef`. Use **Camunda types**: `string`, `boolean`, `integer`, `long`, `double`, `date`, `any`. DMN/FEEL-only types (`number`, `time`, `dateTime`, durations) are **rejected by default** (`types.nonCamundaTypeAction`, §7.1). | `integer` |
...
- `typeRef: "any"` ⇒ omit `typeRef` in the DMN (Camunda shows it as "Any").
...
| `any` (`types.anyKeyword`) | any syntactically valid unary test (e.g. the `tags` expression) | syntax errors only |
...
`typeRef` handling: omit for `any`; otherwise pass through. Only DMN 1.3 built-in FEEL types should reach the writer — validate against the allowed set and warn on unknowns.
...
```jsonc
{
  ...
  // --- type mapping ---
  "types": {
    "anyKeyword": "any",          // this typeRef → omit typeRef in DMN
